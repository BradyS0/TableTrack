...
describe("Restaurant API", () => {
  ...
  it("Get restaurant by owner - user DNE", async () => {
    const res = await request(app).get("/v1/restaurant/user/100").send();
    expect(res.statusCode).toBe(404);
  });
  ...
  it("Change name and description of existing restaurant", async () => {
    const res = await request(app).patch("/v1/restaurant/change").send({
      restID: rest1id,
      name: "Bob's Dairy",
  ...
  it("Create restaurant with invalid data", async () => {
    const res = await request(app).post("/v1/restaurant").send({
      userID: user2id,
      name: "Macdonald",
      address: "nope",
      phone: "(204) 345-6789",
  ...
  it("Change Restaurant tags - valid", async () => {
    ...
    const res = await request(app).patch("/v1/restaurant/change/tags").send({
      restID: rest1id,
      tags: test_tag,
    });
    expect(res.statusCode).toBe(200);
  ...
  it("Restaurant tags - invalid, undefined value", async () => {
    ...
    expect(res.body.error).toBe(
      "Tags are expected as Arrays of strings but [object Undefined] provided"
    );
  });
  ...
  it("Restaurant tags - invalid value", async () => {
    ...
    expect(res.statusCode).toBe(400);
    expect(res.body.error).toBe(
      "invalid syntax, only alphabets and hyphen accepted between 3-30 characters long"
    );
  });
});
...
it("Change name - bad call status 500", async () => {
  const res = await request(app).patch("/v1/restaurant/change/name").send({
    restID: rest1id,
  });
...
it("Change phone - phone num already in use", async () => {
  const res = await request(app).patch("/v1/restaurant/change/phone").send({
    restID: rest1id,
    phone: "(204) 234-5678",
  });
  expect(res.statusCode).toBe(400);
...
it("Change address - address already in use", async () => {
  const res = await request(app).patch("/v1/restaurant/change/address").send({
    restID: rest1id,
    address: "100 Burger street",
  });
  expect(res.statusCode).toBe(400);
